...
$dbTables = array_merge(array(''=>"-- None --"), $dbTables);
	...
	$tips = array(
		...
		, 'btnClearFields'=>"Clear all fields"
...
<div class="modal fade modalEditForm" id="modalEditForm" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body wForm" style="padding-bottom: 0">
            ...
            <hr style="float: none; margin: 0.5em 0" />
              <div style="text-align: center">
                ...
                <div style="display: inline-block">{!! Form::label("chkFormTypeChecklist", "Checklist", [ 'style'=>"float: left; padding: 0.25em 1em 0 1em" ]) !!} {!! Form::radio("chkFormType", "checklist", false, [ 'id'=>"chkFormTypeChecklist" ]) !!}</div>
                {{--<label for="chkFormTypeForm">Form {!! Form::radio("chkFormType", "form", true, [ 'id'=>"chkFormTypeForm" ]) !!} </label>
                <label for="chkFormTypeWorkflow">Workflow {!! Form::radio("chkFormType", "workflow", false, [ 'id'=>"chkFormTypeWorkflow" ]) !!} </label>
                <label for="chkFormTypeChecklist">Checklist {!! Form::radio("chkFormType", "checklist", false, [ 'id'=>"chkFormTypeChecklist" ]) !!} </label>--}}
              </div>
            <div style="clear: both; white-space: nowrap">
            	<h3 class="block-title">Fields</h3>
	            <a class="btn btn-sm" data-toggle="modal" data-target=".modalAddField" id="btnAddField" title="{{$tips['btnAddField']}}">Add</a>
	            <a class="btn btn-sm" data-toggle="modal" data-target=".modalAddField" id="btnClearFields" title="{{$tips['btnClearFields']}}">Clear</a>
	            <a class="btn btn-sm" data-toggle="modal" data-target=".modalAddField" id="btnUpdateFields" title="{{$tips['btnUpdateFields']}}">Update</a>
              @include('forms.toolbar')
            </div>
...
function addField(index, vals) {
	//if (APP_DEBUG > 0)
	console.log("addField(index, vals) index - ",index," (index = 0 - "+(index == 0)+"), vals - ", vals);
	cntFields++;
  ...
  var wrapper = $("#formFields").get(0);
	if (index == -1) $(wrapper).append(template);
	else {
		var fields = $(wrapper).find(".fieldTemplate");
		////if (dbg)
		console.log("  fields ("+fields.length+") - ",fields);
		if (fields.length == 0) $(wrapper).append(template);
      ...
      /*var newIndex = -1;
       ...
       if (el == ui.item.get(0)) newIndex = i;
       });
       console.log("  newIndex - "+newIndex);*/
			$(template).insertBefore(fields[index]);
			//$(wrapper).append(template);
		}
	}
	//$("#formFields").append(template);
...
$(document).ready(function() {
	...
	$("#btnClearFields").on("click", function (ev) { $("#formFields .fieldTemplate").remove(); });
	...
	$("#submitUpdateCustomForm").on("click", function (ev) {
		ev.preventDefault();
		console.log("#submitUpdateCustomForm.click()");
		/*var fields = $("#formFields").find(".fieldTemplate");
		fields.each(function(i, el) {
			console.log("  field: i - ",i,", el - ",el);
			$(el).find("[id^=fieldOrder]").val(i);
		});*/

...
class FormsController extends Controller {
	...
	public function assign($id = null) {
		...
		foreach ($req['users'] AS $uid) {
			...
			if ($queryAssigned->count() == 0) {
				...
				if ($table == "forms_data") $dataNew['form_id'] = $req['form_id'];
				$idNew = -1;
				$idNew = \DB::table($table)->insertGetId($dataNew);
				$txtDebug .= ", \$dataNew - ".print_r($dataNew, 1);
				$txtDebug .= "\n    idNew - ".$idNew;
  ...
  */
  ...
  function getForm($key = null) {
  	$txtDebug = __CLASS__.".".__FUNCTION__."(\$form)";
  	$txtDebug .= PHP_EOL."  \$key - {$key} (numeric - ".is_numeric($key).")";
  	$found = false;
  	$form = null;
  	$forms = null;
	if ($key) {
		if (is_numeric($key)) {
			$form = Form::where("id", "=", $key);
			if ($form->count() > 0) $found = true;
		}

		if (!$found) {
			$form = Form::where("slug", "=", $key);
			if ($form->count() > 0) $found = true;
		}
	} else {
		/*$f = Form::where("id", 1)->first();
		$txtDebug .= PHP_EOL."  \$f - ".($f != null ? print_r($f->toArray(), 1):"");
		die("<pre>{$txtDebug}</pre>");*/
		$forms = Form::all();
		/*$forms = $forms->sort(function($a, $b) {
			$cmp = strcmp($a->name, $b->name);
			//echo "<p>sorting: \$a->name - {$a->name}, \$b->name - {$b->name}, \$cmp - {$cmp}</p>";
			//die("Sorting: <pre>\$a - ".print_r($a, 1)."\n  \$b - ".print_r($b,1)."</pre>");
			return $cmp;
		});*/
//		$forms = $forms->toArray();
//		usort($forms, function($a, $b) { return strcmp($a['name'], $b['name']); });
	...
	if ($form) $form = $form->first();
  	...
  	$txtDebug .= PHP_EOL."  \$found - ".var_export($found, 1);
  	//if ($found) $txtDebug .= ", count - ".$form->count();
  	$txtDebug .= PHP_EOL."  \$form - ".($form != null ? print_r($form->toArray(), 1):"");
  	//$txtDebug .= PHP_EOL."  \$forms - ".($forms != null ? print_r($forms->toArray(), 1):"");
  	$txtDebug .= PHP_EOL."  \$forms - ".($forms != null ? print_r($forms, 1):"");
  	//die("<pre>{$txtDebug}</pre>");
  	//return view('forms.form', compact( 'test',"XYZ" ));
  	return view('forms.form')->with( 'form', $form )->with('forms', $forms);
  }

...
class SubCategoriesController extends Controller
{
    ...
    {
        $subCategories = CaseSubType::select(array('id','name','created_at'))->where('case_type','=',$id);
    ...
    {
         $Subcategory           = new CaseSubType();
         $Subcategory->name     = $request['name'];
         $slug                  = preg_replace('/\s+/','-',$request['name']);
         $Subcategory->slug     = $slug;
         $Subcategory->case_type = $request['subCatID'];
    ...
    {
        $subCategory             = CaseSubType::where('id',$request['subCategoryID'])->first();
        $subCategory->name       = $request['name'];
